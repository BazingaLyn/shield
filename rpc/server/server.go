...
func (s *simpleServer) Register(provider interface{}, metadata map[string]string) error {

	typ := reflect.TypeOf(provider)
	name := typ.Name()
	srv := new(service)
	srv.name = name
	srv.rcvr = reflect.ValueOf(provider)
	...
	if _, loaded := s.serviceMap.LoadOrStore(name, srv); loaded {
...
func (s *simpleServer) Server(network string, addr string) error {
...
func (s *simpleServer) serveTransport(tr transport.Transport) {
	for {
		message, err := protocol.DecodeMessage(s.option.ProtocolType, tr)
		...
		sname := message.ServiceName
		mname := message.MethodName
		...
		err = s.codec.Decode(message.Data, argv)
		...
		responseData, err := codec.GetCodec(message.SerializeType).Encode(replyv)
		if err != nil {
			s.writeErrorResponse(response, tr, err.Error())
			return
		}
